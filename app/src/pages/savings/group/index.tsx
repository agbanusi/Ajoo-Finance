...
import { fetchGroupSavings } from "../../../api/savings";

const GroupSavingsPage: React.FC = () => {
  const [isCreateModalVisible, setIsCreateModalVisible] = useState(false);

  const { data: mockGroupSavings } = useQuery({
    queryKey: ["group savings"],
    queryFn: fetchGroupSavings,
  });
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {mockGroupSavings?.map((group) => (
          <Link to={`/savings/group/${group.id}/details`} key={group.id}>
